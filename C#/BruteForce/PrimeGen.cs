...
namespace LargeCollections
{
    public class LongBitArray
    {
        ...
        {
            Length = length;
            int Amount = (int)Math.Ceiling((double)(length / (Int32.MaxValue-7)));
            int Remainder = (int)(length % (Int32.MaxValue-7));
            LoadedArray = new BitArray(Amount >= 1 ? Int32.MaxValue-7 : Remainder, startValue);
            LoadedArrayIndex = 0;
            mmf = MemoryMappedFile.CreateOrOpen("bits", (long)Math.Ceiling(Length / 8.0)+1);
        ...
        public void Save(int Index)
        {
            ...
            // {"
            //Console.WriteLine((long)Math.Floor((LoadedArray.Length) / 8.0)+" "+ (Math.Floor((Int32.MaxValue / 8.0))) * LoadedArrayIndex+" "+Length/8.0);
            using (var stream = mmf.CreateViewAccessor((long)((Math.Floor((Int32.MaxValue / 8.0))) * LoadedArrayIndex),  (long)Math.Floor((LoadedArray.Length)/8.0)))// (long)Math.Ceiling((Int32.MaxValue / 8.0)) * LoadedArrayIndex
            {
                ...
                stream.WriteArray<byte>(0, buffer, 0, buffer.Length-1);
        ...
        {
            MemoryMappedViewAccessor stream1;//(long)Math.Ceiling(Length / 8.0) - (long)Math.Floor((Int32.MaxValue / 8.0))
            long length1 = (long)Math.Floor((Int32.MaxValue / 8.0)) * (Index + 1) > (long)Math.Ceiling(Length / 8.0) ?  0 : (long)Math.Floor((Int32.MaxValue / 8.0));// * (Index + 1);
            //if (length1 == (long)Math.Floor((Int32.MaxValue / 8.0)) * (Index + 1)) { Console.WriteLine("meme "+length1+" "+Length/8.0); }
            try { stream1 = mmf.CreateViewAccessor((long)(Math.Floor((Int32.MaxValue / 8.0)) * Index), length1); }
            catch (Exception e)
            {
            ...
            int Amount = (int)Math.Ceiling((double)(Length / (Int32.MaxValue-7)));
            int Remainder = (int)(Length % (Int32.MaxValue - 7));
            int length = (int)(Amount == Index ? Math.Ceiling(Remainder / 8.0) : Math.Ceiling((Int32.MaxValue - 7) / 8.0));
            ...
            //stream.Position = 0;//(long)Math.Ceiling((Int32.MaxValue/8.0))*Index;
            //BinaryReader reader = new BinaryReader(stream);
            byte[] buffer = new byte[length];//(int)Math.Floor((Int32.MaxValue / 8.0))];
