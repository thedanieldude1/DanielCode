 	...
 	public static void Main(string[] args){
		...
		for(int y = -resolution/2;y<resolution/2;y++){
			for(int x = -resolution/2;x<resolution/2;x++){
                ...
                double val1 = (gen.GetValue(samp.X,samp.Y,samp.Z,8,0.01,3,2,0.7));
				double biomeval2 = (gen3.GetValue(samp.X,samp.Y,samp.Z,1,0.001,2,2,0.6)+1)/2;
                double val2 = (gen4.GetValue(samp.X, samp.Y, samp.Z, 16, 0.003, 2, 2, .3));
                ...
                double left = vor.GetValue(samp.X + bluramount, samp.Y, samp.Z);
                double right = vor.GetValue(samp.X - bluramount, samp.Y, samp.Z);
                double up = vor.GetValue(samp.X, samp.Y + bluramount, samp.Z);
                double down = vor.GetValue(samp.X, samp.Y - bluramount, samp.Z);
                double pixelsval = vor.GetValue(samp.X, samp.Y, samp.Z);
                double val = (pixelsval+right+left+up+down)/5.0;
                if (val != pixelsval)
                {
                    double downright=0;
                    double downleft=0;
                    ...
                    {
                        //if (i == 0) { continue; };
                        left += vor.GetValue(samp.X + i, samp.Y, samp.Z);
                        right += vor.GetValue(samp.X - i, samp.Y, samp.Z);
                        down += vor.GetValue(samp.X, samp.Y + i, samp.Z);
                        up += vor.GetValue(samp.X, samp.Y + i, samp.Z);
                        downright += vor.GetValue(samp.X - i, samp.Y - i, samp.Z);
                        downleft += vor.GetValue(samp.X + i, samp.Y - i, samp.Z);
                        upright += vor.GetValue(samp.X + i, samp.Y + i, samp.Z);
                        upleft += vor.GetValue(samp.X - i, samp.Y + i, samp.Z);
                    }
                    //left = left + vor.GetValue(samp.X + 2, samp.Y, samp.Z) + vor.GetValue(samp.X + 3, samp.Y, samp.Z) + vor.GetValue(samp.X + 4, samp.Y, samp.Z) + vor.GetValue(samp.X + 1, samp.Y, samp.Z);
                    //right = right + vor.GetValue(samp.X - 4, samp.Y, samp.Z) + vor.GetValue(samp.X - 3, samp.Y, samp.Z) + vor.GetValue(samp.X - 2, samp.Y, samp.Z) + vor.GetValue(samp.X - 1, samp.Y, samp.Z);
                   // up = up + vor.GetValue(samp.X, samp.Y + 4, samp.Z) + vor.GetValue(samp.X, samp.Y + 3, samp.Z) + vor.GetValue(samp.X, samp.Y + 2, samp.Z) + vor.GetValue(samp.X, samp.Y + 1, samp.Z);
                   // down = down + vor.GetValue(samp.X, samp.Y - 4, samp.Z) + vor.GetValue(samp.X, samp.Y - 3, samp.Z) + vor.GetValue(samp.X, samp.Y - 2, samp.Z) + vor.GetValue(samp.X, samp.Y - 1, samp.Z);
                     //= vor.GetValue(samp.X-4, samp.Y - 4, samp.Z) + vor.GetValue(samp.X-3, samp.Y - 3, samp.Z) + vor.GetValue(samp.X-2, samp.Y - 2, samp.Z) + vor.GetValue(samp.X-1, samp.Y - 1, samp.Z);
                    // = vor.GetValue(samp.X + 4, samp.Y - 4, samp.Z) + vor.GetValue(samp.X + 3, samp.Y - 3, samp.Z) + vor.GetValue(samp.X + 2, samp.Y - 2, samp.Z) + vor.GetValue(samp.X + 1, samp.Y - 1, samp.Z);
                    // = vor.GetValue(samp.X - 4, samp.Y + 4, samp.Z) + vor.GetValue(samp.X - 3, samp.Y + 3, samp.Z) + vor.GetValue(samp.X - 2, samp.Y + 2, samp.Z) + vor.GetValue(samp.X - 1, samp.Y + 1, samp.Z);
                   // double upright = vor.GetValue(samp.X + 4, samp.Y + 4, samp.Z) + vor.GetValue(samp.X + 3, samp.Y + 3, samp.Z) + vor.GetValue(samp.X + 2, samp.Y + 2, samp.Z) + vor.GetValue(samp.X + 1, samp.Y + 1, samp.Z);
                    val = (pixelsval + left + right + up +down+downright+downleft+upright+upleft) / (5+8*(bluramount-1));
                }
                else
                {
                    
                }
                //val = chooser.Choose(val1,val2,val);//(val1*biomeval1+val2*biomeval2+val3*biomeval3);
...
public class Voronoi:ValueNoiseBasis{
     public double Frequency { get; set; }
        ...
        public double GetValue(double x, double y, double z)
        {
            ...
            cubex = (cubex > 0.0 ? (int)cubex : (int)cubex - 1);
            cubey = (cubey > 0.0 ? (int)cubey : (int)cubey - 1);
            cubez = (cubez > 0.0 ? (int)cubez : (int)cubez - 1);
            double rightNeighborNoise = ValueNoise(cubex + 1, cubey, cubez);
            double rightNeighborX = cubex + ValueNoise(cubex + 1, cubey, cubez, Seed) - 1;
            double rightNeighborY = cubey + ValueNoise(cubex + 1, cubey, cubez, Seed + 1);
            double rightNeighborZ = cubez + ValueNoise(cubex + 1, cubey, cubez, Seed + 2);
            double rightNeighborDist = (rightNeighborX - xCandidate) * (rightNeighborX - xCandidate) + (rightNeighborY - yCandidate) * (rightNeighborY - yCandidate) + (rightNeighborZ - zCandidate) * (rightNeighborZ - zCandidate);
            double rightDist = (rightNeighborX - x) * (rightNeighborX - x) + (rightNeighborY - y) * (rightNeighborY - y) + (rightNeighborZ - z) * (rightNeighborZ - z);
            double leftNeighborNoise = ValueNoise(cubex - 1, cubey, cubez);
            double leftNeighborX = cubex + ValueNoise(cubex - 1, cubey, cubez, Seed) + 1;
            double leftNeighborY = cubey + ValueNoise(cubex - 1, cubey, cubez, Seed + 1);
            double leftNeighborZ = cubez + ValueNoise(cubex - 1, cubey, cubez, Seed + 2);
            double leftNeighborDist = (leftNeighborX - xCandidate) * (leftNeighborX - xCandidate) + (leftNeighborY - yCandidate) * (leftNeighborY - yCandidate) + (leftNeighborZ - zCandidate) * (leftNeighborZ - zCandidate);
            double leftDist = (leftNeighborX - x) * (leftNeighborX - x) + (leftNeighborY - y) * (leftNeighborY - y) + (leftNeighborZ - z) * (leftNeighborZ - z);
            double upNeighborNoise = ValueNoise(cubex, cubey + 1, cubez);
            double upNeighborX = cubex + ValueNoise(cubex, cubey + 1, cubez, Seed);
            double upNeighborY = cubey + ValueNoise(cubex, cubey + 1, cubez, Seed + 1) + 1;
            double upNeighborZ = cubez + ValueNoise(cubex, cubey + 1, cubez, Seed + 2);
            double upNeighborDist = (upNeighborX - xCandidate) * (upNeighborX - xCandidate) + (upNeighborY - yCandidate) * (upNeighborY - yCandidate) + (upNeighborZ - zCandidate) * (upNeighborZ - zCandidate);
            double upDist = (upNeighborX - x) * (upNeighborX - x) + (upNeighborY - y) * (upNeighborY - y) + (upNeighborZ - z) * (upNeighborZ - z);
            double downNeighborNoise = ValueNoise(cubex, cubey - 1, cubez);
            double downNeighborX = cubex + ValueNoise(cubex, cubey - 1, cubez, Seed);
            double downNeighborY = cubey + ValueNoise(cubex, cubey - 1, cubez, Seed + 1) - 1;
            double downNeighborZ = cubez + ValueNoise(cubex, cubey - 1, cubez, Seed + 2);
            double downNeighborDist = (downNeighborX - xCandidate) * (downNeighborX - xCandidate) + (downNeighborY - yCandidate) * (downNeighborY - yCandidate) + (downNeighborZ - zCandidate) * (downNeighborZ - zCandidate);
            double downDist = (downNeighborX - x) * (downNeighborX - x) + (downNeighborY - y) * (downNeighborY - y) + (downNeighborZ - z) * (downNeighborZ - z);
            double value;
            ...
            double rightDist2 = Math.Sqrt(rightDist) * MathHelper.Sqrt3 / 2;
            double leftDist2 = Math.Sqrt(rightDist) * MathHelper.Sqrt3 / 2;
            double upDist2 = Math.Sqrt(rightDist) * MathHelper.Sqrt3 / 2;
            double downDist2 = Math.Sqrt(rightDist) * MathHelper.Sqrt3 / 2;
            double ratio1 = rightDist > rightNeighborDist ? 0 : 1 / rightDist2;
            double ratio2 = leftDist > leftNeighborDist ? 0 : 1 / leftDist2;// minDist / leftNeighborDist;
            double ratio3 = upDist > upNeighborDist ? 0 : 1 / upDist2;//minDist / upNeighborDist;
            double ratio4 = downDist > downNeighborDist ? 0 : 1 / downDist2;//minDist / downNeighborDist;
            double test = 0;
            test = (double)Math.Max(Math.Max(Math.Max(ratio1, ratio2), ratio3), ratio4);
            double val = 0;
            if (test == ratio1)
            {
                val = rightNeighborNoise;
            }
            else if (test == ratio2)
            {
                //Console.WriteLine("yes");
                val = leftNeighborNoise;
            }
            else if (test == ratio3)
            {
            ...
            {
                val = downNeighborNoise;
            }

            test = 0;

            //Console.WriteLine(rightNeighborNoise + " "+leftNeighborNoise );
            ...
            // ratio4/=totalval;
            value = (value + 1) / 2;
            double normal = 1 / value;
            double totalval = ratio1 + ratio2 + ratio3 + ratio4 + normal;
            ratio1 /= totalval;
            ratio2 /= totalval;
            ratio3 /= totalval;
            ratio4 /= totalval;
            ...
            //if (value < 0) value = 0;
            //normal = Math.Abs(1-ratio1*ratio2*ratio3*ratio4);//Math.Max(Math.Max(Math.Max(ratio1, ratio2), ratio3), ratio4)
