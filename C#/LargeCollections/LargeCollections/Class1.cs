...
namespace LargeCollections
{
    ...
    public class LongBitArray
    {
        ...
        public LongBitArray(long length)
        {
            ...
            LoadedArrayIndex = 0;
            mmf = MemoryMappedFile.CreateOrOpen("bitsc", (long)(Length ));
        ...
        public void Load(int Index)
        {
           ...
           // try {
                stream1 = mmf.CreateViewAccessor((long)(Math.Ceiling((Int32.MaxValue / 8.0)) * Index), (long)Math.Ceiling((Int32.MaxValue / 8.0)) * (Index + 1));// }
           // catch (Exception e)
          //  {
    ...
    public class BigIntBitArray
    {
        ...
        {
            Length = length;
            int Amount = (int)Math.Ceiling((double)(length%Int64.MaxValue / Int32.MaxValue));
        ...
        public void Save(int Index, int ElementIndex)
        {
            ...
            // CustomSecurity.AddAccessRule(new System.Security.AccessControl.AccessRule<MemoryMappedFileRights>("everyone", MemoryMappedFileRights.FullControl, System.Security.AccessControl.AccessControlType
            // .Allow));
            int Amount1 = (int)Math.Ceiling((double)(Length / Int64.MaxValue));
            long Remainder1 = (long)(Length % Int64.MaxValue);
            long length1 = (long)(Amount1 == Index ? Math.Ceiling(Remainder1 / 8.0) : Math.Ceiling(Int64.MaxValue / 8.0));
            using (MemoryMappedFile mmf = MemoryMappedFile.CreateOrOpen("bits" + ElementIndex, length1))
            {
                using (var stream = mmf.CreateViewAccessor((long)(Math.Ceiling((Int32.MaxValue / 8.0)) * LoadedArrayIndex), (long)Math.Ceiling((Int32.MaxValue / 8.0)) * LoadedArrayIndex + (long)Math.Ceiling((Int32.MaxValue / 8.0))))
                {
                    int Amount = (int)Math.Ceiling((double)(Length / Int32.MaxValue));
        ...
        public void Load(int Index, int ElementIndex)
        {
            int Amount1 = (int)Math.Ceiling((double)(Length / Int64.MaxValue));
            long Remainder1 = (long)(Length % Int64.MaxValue);
            long length1 = (long)(Amount1 == Index ? Math.Ceiling(Remainder1 / 8.0) : Math.Ceiling(Int64.MaxValue / 8.0));
            using (MemoryMappedFile mmf = MemoryMappedFile.CreateOrOpen("bits" + ElementIndex, length1))
            {
                using (var stream1 = mmf.CreateViewAccessor((long)(Math.Ceiling((Int32.MaxValue / 8.0)) * Index), (long)Math.Ceiling((Int32.MaxValue / 8.0)) * (Index + 1)))
                {
